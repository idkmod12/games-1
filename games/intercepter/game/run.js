...
let atan2Angles = {};
const Cos = (A) => {
  let a = Number(A.toFixed(1))
  if(cosAngles[a]) return cosAngles[a];
  else {
    cosAngles[a] = cos(a);
    return cosAngles[a]
  }
...
const Sin = (A) => {
  let a = Number(A.toFixed(1))
  if(sinAngles[a]) return sinAngles[a];
  else {
    sinAngles[a] = sin(a);
    return sinAngles[a]
  }
...
const Atan2 = (y, x) => {
  let Y = Number(y.toFixed(1));
  let X = Number(x.toFixed(1));
  if(atan2Angles[Y + "--" + X]) return atan2Angles[Y + "--" + X];
  else {
    atan2Angles[Y + "--" + X] = atan2(Y, X);
    return atan2Angles[Y + "--" + X];
  }
}
...
function windowResized() {
  //esizeCanvas(constrain(windowWidth, 600, 1800), constrain(windowHeight, 500, 1000));
}